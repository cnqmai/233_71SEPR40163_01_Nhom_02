...
import styles from "./GroupYourGroup.module.css";

const GroupYourGroup = () => {
  ...
  const fetchManagedGroups = async () => {
    ...
    try {
      ...
      setManagedGroups(response.data || []); // Đảm bảo là mảng
    } catch (error) {
  ...
  const fetchParticipatedGroups = async () => {
    ...
    try {
      ...
      setParticipatedGroups(response.data || []); // Đảm bảo là mảng
    } catch (error) {
  ...
  const GroupCard = ({ group, onViewClick, onLeaveClick }) => (
    group ? (
      <form className={styles.card}>
        <img className={styles.cardChild} src={`/images/${group?.AnhDaiDien || 'default-image.png'}`} alt={group?.TenNhom || 'Group Image'} />
        <div className={styles.content}>
          <div className={styles.info6}>
            <div className={styles.tnNhm6}>{group?.TenNhom || 'Unknown Group'}</div>
            <div className={styles.mTGii}>{group?.GioiThieu || 'No description'}</div>
          </div>
          <div className={styles.buttonCon}>
            <button className={styles.button3} onClick={() => onViewClick(group)}>
              <div className={styles.title1}>Xem nhóm</div>
            </button>
            <button className={styles.button3} onClick={() => onLeaveClick(group)}>
  ...
  return (
    <div className={styles.groupyourgroup}>
      <TopNavCom />
      ...
      <div className={`${styles.panel} ${isLeftExpanded ? styles.expanded : styles.collapsed}`}>
          ...
          <div className={styles.nhm1}>
            ...
            <div className={styles.list}>
              {managedGroups.slice(0, 3).map(group => (
                <button key={group.id} className={styles.item} onClick={() => handleGroupNavigation(`/groupthaoluan/${userId}/${group?.TenNhom || 'unknown'}`)}>
                  <div className={styles.imageCon}>
                    <img className={styles.image20} src={`/images/${group?.AnhDaiDien || 'default-image.png'}`} alt={group?.TenNhom || 'Group'} />
                  </div>
                  <div className={styles.info}>
                    <div className={styles.tnNhm}>{group?.TenNhom || 'Unknown Group'}</div>
                    {/* <div className={styles.giiThiu}>{group?.GioiThieu || 'No description'}</div> */}
                  </div>
                </button>
              ))}
            </div>
        ...
        <section className={styles.container}>
          <div className={styles.heading2}>
            <div className={styles.nhmCaBn}>Nhóm của bạn</div>
          </div>
          <div className={styles.list2}>
            {participatedGroups.map(group => (
              <GroupCard
                key={group.id}
                group={group}
                onViewClick={group => handleGroupNavigation(`/groupthaoluan/${userId}/${group?.TenNhom || 'unknown'}`)}
